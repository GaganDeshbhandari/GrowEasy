          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto flex-1">
            ...
            <div className="relative w-full sm:w-auto group">
              <select
                ...
                className="w-full sm:w-56 pl-5 pr-10 py-3 rounded-2xl border border-gray-200/70 dark:border-gray-700/70 bg-white dark:bg-gray-800 text-sm font-bold text-gray-700 dark:text-gray-200 focus:outline-none focus:ring-4 focus:ring-emerald-500/20 focus:border-emerald-500 hover:border-gray-300 dark:hover:border-gray-600 transition-all cursor-pointer appearance-none shadow-sm"
              >
                {allCategories.map((cat) => (
                  <option key={cat.id} value={cat.id}>{cat.name}</option>
                ))}
              </select>
              <svg className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 group-hover:text-gray-600 dark:group-hover:text-gray-300 transition-colors pointer-events-none" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
            </div>
          </div>

          {/* Results count */}
          <div className="text-xs font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest whitespace-nowrap bg-gray-100/50 dark:bg-gray-800/50 px-4 py-3 rounded-xl border border-gray-200/50 dark:border-gray-700/50">
            Showing <span className="text-emerald-700 dark:text-emerald-400 text-sm">{filtered.length}</span> of {count}
          </div>
        </div>
      </div>
                ...
                <div className="p-6 flex flex-col flex-grow relative bg-white dark:bg-gray-900">
                  ...
                  <div className="mt-6 pt-5 border-t border-gray-100 dark:border-gray-800 flex items-end justify-between">
                    ...
                    <span className={`text-[10px] font-black uppercase tracking-widest px-2.5 py-1.5 rounded-[0.4rem] border ${
                      ...
                      {parseFloat(product.stock) > 0 ? `${parseFloat(product.stock)} Left` : "Out of Stock"}
                    </span>
                  </div>
                </div>
              </div>
